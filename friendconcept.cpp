...
class Fraction{
    ...
    friend Fraction  addMixedFraction(const Fraction& s1,const Fraction& s2);
};
Fraction addMixedFraction(const Fraction& s1,const Fraction& s2){
    int sum,er;
   if(s1.denominator==s2.denominator){
       sum=s1.numerator+s2.numerator;
       er=s1.denominator;
   }else if(s1.denominator!=s2.denominator){
       sum=(s1.numerator * s2.denominator)+(s2.numerator * s1.denominator);
       er=s2.denominator * s1.denominator;
   }
   return   Fraction(sum,er);
...
int main(){
    int p,q,r,s;
    cin>>p>>q>>r>>s;
    Fraction s1(p,q),s2(r,s);
    Fraction pi=addMixedFraction(s1,s2);
    pi.display();
