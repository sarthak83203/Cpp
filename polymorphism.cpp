...
class Caluclated{
    public:
    //car speed
    double calculateSpeed(double distance,double times){
        return (distance/times);
    }
    double calculateSpeed(double distance,double times, double acceleration1){
        return ((distance/times)+ acceleration1 * times); 
    }
   
    
};
int main(){
    double distance,times,acceleration1,acceleration2;
    cin>>distance>>times>>acceleration1>>acceleration2;
    Caluclated s1;
    cout<<fixed<<setprecision(2)<<s1.calculateSpeed(distance,times)<<endl;
     cout<<fixed<<setprecision(2)<<s1.calculateSpeed(distance,times,acceleration1)<<endl;
      cout<<fixed<<setprecision(2)<<s1.calculateSpeed(distance,times,acceleration2)<<endl;
    return 0;
}